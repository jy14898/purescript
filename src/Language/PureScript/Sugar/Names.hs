...
renameInModule imports (Module modSS coms mn decls exps) =
  ...
  updateTypesEverywhere = everywhereOnTypesM updateType
    ...
    updateInConstraint (Constraint ann@(ss, _) name ks ts info mul) =
      Constraint ann <$> updateClassName name ss <*> pure ks <*> pure ts <*> pure info <*> pure mul

  updateConstraints :: SourceSpan -> [SourceConstraint] -> m [SourceConstraint]
  updateConstraints pos = traverse $ \(Constraint ann name ks ts info mul) ->
    Constraint ann
